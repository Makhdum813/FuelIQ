...
// Reliable "back to landing page" behaviour.
// When the app is opened from the landing page, the normal browser Back button
// should return there. Some setups (PWA cache, standalone installs) can make
// plain Back unreliable, so we push a history entry on load and, if the user
// presses Back while still on the app, send them to the landing page.
(function(){
  try{
    // Seed one extra history state so a Back press fires popstate here first,
    // rather than doing nothing or leaving the user stuck.
    history.pushState({ fueliqApp: true }, '', location.href);
    window.addEventListener('popstate', function(){
      // User pressed Back while on the app -> go to the landing page one level up.
      window.location.href = '../index.html';
    });
  }catch(e){ /* history API unavailable — the sidebar "Back to Home" link still works */ }
})();
